group('graphics', function()
  ...
  group('Pass', function()
    ...
    group(':setCanvas', function()
      ...
      test('switching between textures without temporary MSAA attachments', function()
        texture1 = lovr.graphics.newTexture(1, 1)
        texture2 = lovr.graphics.newTexture(1, 1)
        pass = lovr.graphics.newPass()
        pass:setCanvas({ texture1, samples = 1 })
        pass:setCanvas({ texture2, samples = 1 })
      end)
    end)
  ...
  group('Readback', function()
    test('poll', function()
      buffer = lovr.graphics.newBuffer('int')
      buffer:setData(100)
      readback = buffer:newReadback()
      lovr.graphics.submit()
      while not readback:isComplete() do
        lovr.timer.sleep(.001)
      end
      expect(readback:getData()).to.equal(100)
    end)
    ...
    test('wait', function()
      ...
      readback = buffer:newReadback()
      readback:wait()
      expect(readback:getData()).to.equal(97)
    end)
    ...
    test('blob', function()
      ...
      readback = buffer:newReadback()
      readback:wait()
      expect(readback:getBlob():getU32()).to.equal(98)
    end)

    test('image', function()
      image = lovr.data.newImage(2, 2)
      image:setPixel(0, 0, 1, 0, 0, 1)
      image:setPixel(1, 0, 0, 1, 0, 1)
      image:setPixel(0, 1, 0, 0, 1, 1)
      image:setPixel(1, 1, 1, 1, 1, 1)
      texture = lovr.graphics.newTexture(image, { usage = 'transfer' })
      readback = texture:newReadback()
      readback:wait()
      expect(readback:getImage()).to_not.equal(image)
      expect({ readback:getImage():getFormat() }).to.equal({ image:getFormat() })
      expect({ readback:getImage():getDimensions() }).to.equal({ image:getDimensions() })
      expect({ readback:getImage():getPixel(0, 0) }).to.equal({ image:getPixel(0, 0) })
      expect({ readback:getImage():getPixel(1, 0) }).to.equal({ image:getPixel(1, 0) })
      expect({ readback:getImage():getPixel(0, 1) }).to.equal({ image:getPixel(0, 1) })
      expect({ readback:getImage():getPixel(1, 1) }).to.equal({ image:getPixel(1, 1) })
    end)

    test('gc', function()
      do
        local buffer = lovr.graphics.newBuffer('int')
        buffer:setData(101)
        buffer:newReadback()
        lovr.graphics.submit()
        buffer:setData(102)
